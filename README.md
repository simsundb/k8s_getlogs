...
2. **② 日志抓取**：选主机→自动连→命名空间→默认全不选，全选/取消全选，部署名可多选（勾选即选中该部署全部 Pod），支持单 Pod 勾选，搜索框在已选基础上过滤 → 日志类别（ALL/hycommon/hyframework）＋日志目录（默认 `/opt/logs`，可改，去该目录取日志）＋日志名（非空时匹配包含该名的 `.log`）→ 并发采集（采集到的日志文件本身按 `<部署名>_<文件名>` 命名，落在 `<日期>/<命名空间>/<部署名>/<Pod>/`）→ zip + manifest；输出框逐 Pod 显示文件数与总大小，结束时汇总。勾选「采集后汇总日志到存储目录」（默认开启）时，采集完成后把所有 `.log` 按 `<部署名>_<文件名>` 打平复制到存储目录下 `logs_collected/<日期>/`（存储目录默认 = 软件所在目录，可另行指定；跨 Windows / macOS），并附 `_copy_log.txt` 来源清单
