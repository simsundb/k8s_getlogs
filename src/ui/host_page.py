...
class HostPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self.ip_edit.setToolTip("MASTER 主机 IP 或域名")
        form.addWidget(self.ip_edit)
        form.addWidget(QLabel("端口:"))
        self.port_edit = QLineEdit("22")
        self.port_edit.setToolTip("SSH 端口，默认 22")
        form.addWidget(self.port_edit)
        form.addWidget(QLabel("账号:"))
        self.user_edit = QLineEdit()
        self.user_edit.setPlaceholderText("例如 root")
        self.user_edit.setToolTip("SSH 登录账号（常用 root）")
        form.addWidget(self.user_edit)
        form.addWidget(QLabel("密码:"))
        self.pwd_edit = QLineEdit()
        self.pwd_edit.setEchoMode(QLineEdit.Password)
        self.pwd_edit.setPlaceholderText("••••••••")
        self.pwd_edit.setToolTip("SSH 密码，仅以 base64 混淆存于本地配置，不记录明文")
        form.addWidget(self.pwd_edit)
        form.addWidget(QLabel("备注:"))
        self.remark_edit = QLineEdit()
        self.remark_edit.setPlaceholderText("可选，例如「生产-MASTER」")
        self.remark_edit.setToolTip("备注便于区分多台主机")
        form.addWidget(self.remark_edit)

        btn_row = QHBoxLayout()
        self.add_btn = QPushButton("新增")
        self.add_btn.setProperty("primary", True)
        set_icon(self.add_btn, "plus", color="white")
        self.add_btn.setToolTip("保存当前表单为新主机")
        self.update_btn = QPushButton("更新")
        set_icon(self.update_btn, "edit-2")
        self.update_btn.setToolTip("用表单内容覆盖表格中选中的主机")
        self.del_btn = QPushButton("删除")
        set_icon(self.del_btn, "trash-2")
        self.del_btn.setToolTip("删除表格中选中的主机")
        btn_row.addWidget(self.add_btn)
        btn_row.addWidget(self.update_btn)
        btn_row.addWidget(self.del_btn)
        form.addLayout(btn_row)
        self.test_btn = QPushButton("测试连接")
        set_icon(self.test_btn, "zap")
        self.test_btn.setToolTip("用表单中的主机信息测试 SSH 连接是否可达")
